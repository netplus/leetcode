...
//
// 5. 专项算法｜奇偶映射 + 前缀和 + 频次哈希表（复用 LC-560）
//   到这里才需要给这套规律命名：本题对应“奇偶映射 + 前缀和 + 频次哈希表（复用 LC-560）”。通用机制见 docs/algorithms/prefix-sum-difference.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
