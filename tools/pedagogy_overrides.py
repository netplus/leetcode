...
public:
    ...
    53: {
        "pattern": "从枚举所有区间到 Kadane：只保留最有前途的结尾状态",
        "model": "最直接的做法是枚举每个起点，再不断向右扩展终点并累计区间和，时间 O(n^2)。优化的关键不是先背 Kadane 公式，而是换一个观察方向：当终点固定为 i 时，所有“以 i 结尾”的候选中，只需要保留和最大的那个。因为未来无论再接上什么元素，这些候选都会加上完全相同的后缀；当前已经更小的候选以后永远不可能反超。于是原本大量区间候选可以压缩成一个 current 状态。",
        "visual": """先从最自然的枚举开始。例：
...
current 只回答“以当前位置结尾的最好是多少”；best 才保存整个扫描过程见过的全局最大值。""",
        "core": "暴力会保留很多以同一位置结尾的候选；由于未来给它们追加的是同一批元素，只需留下当前和最大的那个，再决定它值得继续带上还是从当前位置重新开始。",
        "formula": """最直接的 O(n^2) 枚举：
...
旧 current == 0 时两种选择等价。""",
        "steps": [
            "先想清楚暴力：枚举 left，向右扩 right，用 running sum 检查所有 O(n^2) 个连续区间",
            "改为从左到右枚举终点 i，并令 current 表示“必须以 i 结尾”的最大子数组和",
            "处理 nums[i] 时，只比较两种合法来源：从 nums[i] 重开，或把 nums[i] 接到上一位置的 current 后面",
            "执行 current=max(nums[i], current+nums[i])，把所有以 i 结尾的候选再次压缩成一个最优状态",
            "执行 best=max(best,current)，因为全局答案可能在任何一个终点结束，而 current 之后还会继续变化",
        ],
        "proof": "固定终点 i，任何非空连续子数组要么只含 nums[i]，要么由一个以 i-1 结尾的连续子数组加上 nums[i] 得到。对第二类而言，所有候选都加同一个 nums[i]，因此上一位置和更小的候选不可能在加法后超过上一位置的最大候选；只保留 current 不会丢掉未来可能成为最优解的状态。于是 current 精确覆盖“以 i 结尾”的最优答案，再对所有终点的 current 取最大，就得到全局最大子数组和。",
        "pitfalls": "不要把 current 和 best 混为一谈：样例扫描到 -5 时 current 从 6 降到 1，但历史最优 best 仍是 6。current/best 必须用 nums[0] 初始化，不能初始化为 0，否则全负数组会错误选择不存在的空子数组；例如 [-5,-2,-3,-1,-4] 的答案应为 -1。所谓“负数就丢掉”更准确地说是“旧 current<0 时不再把它接到当前元素前面”，并不是数组里遇到负数就一定断开。当前约束下最大和不超过 10^9，int 足够；更宽泛的变体可改用 long long。",
        "transfer": "这题最值得迁移的是“按终点组织候选 + 丢掉被支配状态”的动态规划视角。以后遇到连续区间最优问题，可以先问：固定当前终点后，过去有哪些候选会面对完全相同的未来？如果一个状态在所有未来扩展下都不可能反超另一个，就可以安全淘汰。最大乘积子数组之所以要同时保留最大值和最小值，正是因为乘负数会反转大小关系，不能像本题一样只留一个最大状态。",
        "memory": "先枚举区间理解问题 -> 改按终点看 -> 同一终点只留最大候选 -> current 决定接上还是重开 -> best 记录所有终点里的最大值。",
        "advanced": "如果题目还要求返回最大子数组的左右边界，可以在“从当前位置重开”时记录新的 candidateLeft，并在 best 被 current 刷新时保存 bestLeft/bestRight；Kadane 的状态压缩逻辑本身不变。",
        "code": r'''// ---------- Solution ----------
class Solution {
public:
    int maxSubArray(vector<int>& nums) {
        // current：必须以当前扫描位置结尾的最大子数组和。
        // best：截至当前为止，所有结尾位置中出现过的最大值。
        int current = nums[0];
        int best = nums[0];

        for (int i = 1; i < static_cast<int>(nums.size()); ++i) {
            // 所有以 i 结尾的区间只有两种最优来源：
            // 1) 从 nums[i] 重新开始；2) 接在上一位置的最优后缀 current 后。
            current = max(nums[i], current + nums[i]);

            // current 只代表“以 i 结尾”；全局最优可能早已在前面结束。
