...
//
// 2. 最自然的一般性解法
//   从给定起点沿边遍历可达节点。第一次遇到原节点 x 时创建唯一克隆 x'；随后复制 x 的每条邻接边，把它连接到对应邻居的克隆。若邻居尚未创建，就先创建并安排后续处理。
//
// 3. 一般解法的问题与限制
//   图可能有环，也可能多条路径到达同一节点。若每次遇见节点都 new 一个副本，会把一个对象复制成多个；若不记录已处理状态，在环上还会无限重复。
//
// 4. 从具体问题抽象规律
//   真正需要保存的是对象身份映射 original -> clone。它同时解决‘是否已经见过’和‘其它边应该连接到哪个唯一副本’两个问题。
//
// 5. 必要背景知识
...
//   前面的推理已经得到可复用机制；现在才给它一个检索名称：图遍历 + 对象身份哈希映射。通用机制见 docs/algorithms/graph-search-toposort.md。回到本题时，只需要把其中的状态、边界和更新事件映射到当前题意，不要反过来用模板猜题。
//
// 7. 核心算法
