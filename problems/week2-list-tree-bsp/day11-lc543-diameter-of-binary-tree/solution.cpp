...
//
// 5. 必要背景知识
//   直径（diameter）是树中任意两个节点之间最长简单路径的长度；LC-543 明确按“边数”计，不按节点数计。
//
...
//
// 6. 专项算法｜树形 DP 双角色：向父只能交一条臂，当前节点却可以把左右两臂拼成完整路径
//   到这里才需要给这套规律命名：本题对应“树形 DP 双角色：向父只能交一条臂，当前节点却可以把左右两臂拼成完整路径”。通用机制见 docs/algorithms/tree-traversal.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 7. 核心算法
