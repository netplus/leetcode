...
//
// 2. 最自然的一般性解法
//   可以分别找到 root 到 p、root 到 q 的两条路径，再从头比较，最后一个相同节点就是答案。这个方法直观且正确，但需要保存路径/父指针信息。
//
// 3. 一般解法的问题与限制
//   两条路径法会显式保存从根到目标的历史。其实站在某个节点时，只需要知道‘p/q 是否出现在左子树、右子树或当前节点’，无需保留完整路径；这些信息可以由子树返回值直接汇总。
//
// 4. 从具体问题抽象规律
//   若当前节点本身是目标，就把自己向上报告；否则分别询问左右子树。左右都报告非空，说明两个目标第一次在这里汇合；只有一边非空，就把那边继续向上传。
//
// 5. 必要背景知识
//   LC-236 里的 p、q 是两个确定的节点对象（`TreeNode*`），不是两个“节点值”。题目本身还保证所有 Node.val 互不相同，但算法语义仍然应该按节点身份理解。
//
//   两个确定节点可以有多个公共祖先。例如：
//
//           3
...
//
// 6. 专项算法｜后序证据汇聚：子树向上报告 p/q，第一次左右同时有证据的位置就是 LCA
//   前面的推理已经得到可复用机制；现在才给它一个检索名称：后序汇总定位最近公共祖先。通用机制见 docs/algorithms/tree-traversal.md。回到本题时，只需要把其中的状态、边界和更新事件映射到当前题意，不要反过来用模板猜题。
//
// 7. 核心算法
