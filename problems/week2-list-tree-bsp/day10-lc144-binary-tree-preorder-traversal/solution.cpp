...
//
// 2. 最自然的一般性解法
//   把栈看成“未来待进入的子树清单”，而不是把前序遍历记成一套固定模板。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：最常见错误是把左右孩子的压栈顺序写成 left 再 right，这会得到根-右-左。
//
// 4. 从具体问题抽象规律
//   Day 10 的 LC-94/145 只是在改变“什么时候处理根”。
//
// 5. 专项算法｜前序遍历：根先处理，栈只负责保存之后要走的分支
//   到这里才需要给这套规律命名：本题对应“前序遍历：根先处理，栈只负责保存之后要走的分支”。通用机制见 docs/algorithms/tree-traversal.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
