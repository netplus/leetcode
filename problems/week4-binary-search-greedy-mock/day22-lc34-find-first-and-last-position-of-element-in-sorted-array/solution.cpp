...
//
// 5. 专项算法｜边界二分：从找一个 target 升级为找单调谓词第一次成立的位置
//   到这里才需要给这套规律命名：本题对应“边界二分：从找一个 target 升级为找单调谓词第一次成立的位置”。通用机制见 docs/algorithms/binary-search.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
