...
//
// 6. 边界与易错点
//   reachable[0] 不能漏掉，否则所有切分都没有起点。字典词允许重复使用，所以这里不是 LC-416 那种“每个物品一次”的倒序背包；同一个 word 可以从不同 start 多次形成边。substr 会产生临时字符串，约束下足够；若追求更强性能可改 trie 或基于索引的比较，但不要无意引入 C++20 API。
//
// 7. 举一反三
