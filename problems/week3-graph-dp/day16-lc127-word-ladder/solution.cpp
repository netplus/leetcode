...
//
// 5. 专项算法｜隐式无权图最短路：按规则现场生成邻居，而不是先显式两两建图
//   到这里才需要给这套规律命名：本题对应“隐式无权图最短路：按规则现场生成邻居，而不是先显式两两建图”。通用机制见 docs/algorithms/graph-search-toposort.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
