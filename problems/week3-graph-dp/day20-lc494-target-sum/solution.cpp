...
//
// 5. 专项算法｜0/1 背包计数：先用代数把正负号选择变成 LC-416 的子集和
//   到这里才需要给这套规律命名：本题对应“0/1 背包计数：先用代数把正负号选择变成 LC-416 的子集和”。通用机制见 docs/algorithms/dynamic-programming.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
