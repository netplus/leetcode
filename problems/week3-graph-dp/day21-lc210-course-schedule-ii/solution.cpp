...
//
// 2. 最自然的一般性解法
//   完全复用 LC-207：prerequisites=[a,b] 建边 b->a，indegree[a] 记录尚未完成的先修数量。队列中的课程都是当前依赖已清零、可以立即学习的 frontier。LC-207 只统计最终处理数来回答能否完成；本题在每次出队时把课程追加到 order，于是同一消除过程本身就是一份合法拓扑序。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：最常见错误仍是把 [a,b] 建成 a->b。
//
// 4. 从具体问题抽象规律
//   这是 Day 15 LC-207 的零新模板迁移：同一个 Kahn 过程，LC-207 的输出是 bool，LC-210 的输出是过程轨迹 order。
//
// 5. 专项算法｜LC-207 的输出型迁移：Kahn 拓扑排序把可完成性证明直接变成课程顺序
//   到这里才需要给这套规律命名：本题对应“LC-207 的输出型迁移：Kahn 拓扑排序把可完成性证明直接变成课程顺序”。通用机制见 docs/algorithms/graph-search-toposort.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
