...
//
// 2. 最自然的一般性解法
//   先从最直观的模拟出发：建立 answer[1..n]，每条 booking=[first,last,seats] 就把 first..last 每个航班都 +=seats。差分没有改变这件事的含义，它只是把一段连续、完全相同的重复写入压缩成两个“状态变化事件”。真正让这种压缩成立的是最后那次从左到右的累积：running 会继承上一个位置的值，因此一次 +seats 可以自动持续影响后续位置，直到遇到对应的 -seats 才把这份状态撤销。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：不要把 diff 当成每个航班最终的座位数，也不要把 -seats 理解成业务上的负座位。
//
// 4. 从具体问题抽象规律
//   这其实是一类通用的“事件驱动 + 累积状态”技巧：公交上车时人数加入当前人数、下车后移除；会议开始时 active meeting +1、结束时 -1；扫描线进入区间时加入贡献、离开时删除贡献。
//
// 5. 专项算法｜差分数组：区间更新转端点更新
//   到这里才需要给这套规律命名：本题对应“差分数组：区间更新转端点更新”。通用机制见 docs/algorithms/prefix-sum-difference.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
