...
//
// 1. 题目直觉 / 图形模型
//   先沿用 LC-141 找到环内相遇点 M：╲n╲nhead -- a步 --> ENTRY -- b步 --> M╲n                 ^               |╲n                 |               |╲n                 +---- c步 <-----+╲n╲n环长 L = b + c。╲n╲n第一阶段看起来只告诉我们“有环”，但相遇发生的路程其实已经藏着入口信息。╲n相遇后把 slow 放回 head，fast 留在 M，然后两者都改成每次走 1 步：╲n╲nslow: head  -------- a步 --------> ENTRY╲nfast: M ---- c步 + 若干整圈 ------> ENTRY╲n╲n两段路程恰好等长，所以它们下一次相遇的位置就是 ENTRY。
//
// 2. 最自然的一般性解法
...
//
// 5. 专项算法｜Floyd 两阶段：从环内相遇点反推出入口
//   到这里才需要给这套规律命名：本题对应“Floyd 两阶段：从环内相遇点反推出入口”。通用机制见 docs/algorithms/linked-list-pointer.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
