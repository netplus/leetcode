...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：题目要求拼接原链表节点，不需要复制节点值；相等时从任一边取都能保持非递减。
//
// 4. 从具体问题抽象规律
//   这是归并排序 merge 阶段的链表版本。
//
// 5. 专项算法｜哑节点 + 双路归并
//   到这里才需要给这套规律命名：本题对应“哑节点 + 双路归并”。通用机制见 docs/algorithms/linked-list-pointer.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
