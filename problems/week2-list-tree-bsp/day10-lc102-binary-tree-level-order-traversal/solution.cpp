...
//
// 2. 最自然的一般性解法
//   从根开始维护‘当前还没处理的一层节点’。先记下这一层有多少个节点，把它们全部取出、记录值，并把各自孩子加入下一批；当前批处理完后再进入下一层。
//
// 3. 一般解法的问题与限制
//   如果只是普通深度优先地访问节点，却不额外携带 depth，就无法知道何时一层结束。这里必须让待处理状态天然按距离分层，或显式记录每个节点深度。
//
// 4. 从具体问题抽象规律
//   当问题关心‘离起点多少步/多少层’时，应该维护 frontier：当前层全部处理完，下一批新发现状态才构成下一层。
//
// 5. 专项算法｜层序遍历：BFS 队列 + 冻结当前层宽度
//   前面的推理已经得到可复用机制；现在才给它一个检索名称：按层 frontier 的队列遍历。通用机制见 docs/algorithms/tree-traversal.md。回到本题时，只需要把其中的状态、边界和更新事件映射到当前题意，不要反过来用模板猜题。
//
// 6. 核心算法
