...
//
// 5. 专项算法｜有序数组的相向双指针
//   到这里才需要给这套规律命名：本题对应“有序数组的相向双指针”。通用机制见 docs/algorithms/two-pointers.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
...
//
// 9. 为什么不会漏 / 不会重
//   每次移动指针前，都能用有序性证明被丢弃的端点不可能和当前候选区间中的任何元素组成 target，因此不会漏解；每一步至少丢弃一个端点，所以最多线性扫描一次。
//
// 10. 边界与易错点
