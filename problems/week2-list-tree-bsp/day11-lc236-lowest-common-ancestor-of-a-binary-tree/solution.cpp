...
//
// 2. 最自然的一般性解法
//   把 Day 10 的“子树向父返回状态”升级为“子树向父返回证据”；父节点只根据左右证据是否同时出现做决策。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：这里比较的是节点身份 root==p/root==q，而不是只比较 val；虽然本题值唯一，但 LeetCode 方法签名语义仍是节点。
//
// 4. 从具体问题抽象规律
//   这是一类非常通用的后序汇聚：子树返回“证据”，父节点在证据首次同时出现时完成答案。
//
// 5. 必要背景知识
...
//   到这里才需要给这套规律命名：本题对应“后序证据汇聚：子树向上报告 p/q，第一次左右同时有证据的位置就是 LCA”。通用机制见 docs/algorithms/tree-traversal.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 7. 核心算法
