...
//
// 2. 最自然的一般性解法
//   最自然的正确算法，是从每个 left 出发重新向右累加，找到第一个 sum>=target 的 right；最坏会为不同 left 重复扫描大量相同元素，达到 O(n^2)。
//
// 3. 一般解法的问题与限制
...
//
// 5. 专项算法｜正数数组上的可变滑动窗口
//   到这里才需要给这套规律命名：本题对应“正数数组上的可变滑动窗口”。通用机制见 docs/algorithms/sliding-window.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
...
//
// 9. 为什么不会漏 / 不会重
//   固定一个 right 时，随着 left 右移，正数保证窗口和只会下降。因此 while 循环会从较长合法窗口一路缩到最短合法窗口，再多缩一步就非法；这个 right 下不可能还有更短合法起点被漏掉。left/right 都只向右移动，所以总复杂度 O(n)。
//
// 10. 边界与易错点
//   正数条件是整个算法成立的根基；若含负数，删掉左端后 sum 可能反而变大，窗口就失去单调性。更新 best 必须发生在移除左端之前；sum 用 long long 更稳妥。
//
// 11. 举一反三
//   判断能否用滑动窗口时，不要先看题目有没有“子数组”，而要先问：扩右、缩左时目标状态是否具有单调性。含负数的最短子数组和需要换前缀和/单调队列等方法。
// ----------------------------------------------------------------------------
//
// 本地输入输出格式（用于 test.in）：
//   第 1 行：n target。
