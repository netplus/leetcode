...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：“领先 n 还是 n+1”不能死记，要先明确 slow 最终要停在目标还是目标前驱。
//
// 4. 从具体问题抽象规律
//   它和 LC-141 的快慢指针外形相似，但目的不同：LC-141 利用速度差，LC-19 维护固定距离。
//
// 5. 专项算法｜哑节点 + 固定间距双指针定位倒数位置
//   到这里才需要给这套规律命名：本题对应“哑节点 + 固定间距双指针定位倒数位置”。通用机制见 docs/algorithms/linked-list-pointer.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
