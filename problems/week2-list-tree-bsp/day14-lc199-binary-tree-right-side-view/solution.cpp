...
//
// 2. 最自然的一般性解法
//   这题几乎不需要新算法。直接复用 Day 10 / LC-102 的 BFS 层边界：一轮开始时 queue 恰好保存当前层全部节点，先冻结 width，再处理这 width 个节点。因为当前实现按左孩子、右孩子入队，所以这一层最后被处理的节点就是最右节点；LC-199 只是把 LC-102 的“收集整层”改成“只收集层末一个值”。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：不要在 for 条件里动态使用 `pending.size()` 作为当前层长度，因为处理中会持续加入下一层孩子；必须先冻结 width。
//
// 4. 从具体问题抽象规律
//   它是 LC-102 的最小改造：BFS 框架、frontier、width 不变量全部不变，只改变“每层如何聚合”。
//
// 5. 专项算法｜层序遍历的层内聚合：每层只取最右一个节点
//   到这里才需要给这套规律命名：本题对应“层序遍历的层内聚合：每层只取最右一个节点”。通用机制见 docs/algorithms/tree-traversal.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
