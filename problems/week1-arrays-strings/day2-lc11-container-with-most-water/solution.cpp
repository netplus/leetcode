...
//
// 5. 专项算法｜相向双指针：移动短板
//   到这里才需要给这套规律命名：本题对应“相向双指针：移动短板”。通用机制见 docs/algorithms/two-pointers.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
...
//
// 9. 为什么不会漏 / 不会重
//   当左边是短板时，所有保留 left、只缩短右边界的候选都被当前组合支配：宽度更小，且高度上限不会超过 height[left]。因此丢弃 left 不会错过更优解；右边较短时完全对称。
//
// 10. 边界与易错点
//   不要移动较高的一端，那无法突破当前短板上限；宽度是 right-left；面积乘法最好先提升到 long long；这题虽然也是相向双指针，但依据是“瓶颈支配关系”，不同于 LC-167 的有序和单调性。
//
// 11. 举一反三
