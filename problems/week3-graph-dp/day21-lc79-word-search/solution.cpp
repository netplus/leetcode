...
//
// 2. 最自然的一般性解法
//   把每个可能匹配 word[0] 的格子当作起点，沿上下左右尝试下一个字符。进入一个格子时记录‘当前路径已使用’，匹配下一个字符；若失败，返回上一格并恢复该标记，再尝试其它方向。
//
// 3. 一般解法的问题与限制
//   不同路径会共享大量格子，搜索规模最坏仍是指数级；但 visited 不能做成全局永久标记，因为同一格可以属于另一条独立候选路径，只是不能在同一条路径中重复使用。
//
// 4. 从具体问题抽象规律
//   当约束只对‘当前构造路径’生效时，状态必须随递归进入和退出而变化：进入时做选择/标记，退出时撤销。这与连通分量搜索的一次性 visited 不同。
//
// 5. 专项算法｜路径回溯：把 Day 13 的选择-递归-撤销迁移到二维网格
//   前面的推理已经得到可复用机制；现在才给它一个检索名称：网格回溯：路径局部 visited。通用机制见 docs/algorithms/backtracking.md。回到本题时，只需要把其中的状态、边界和更新事件映射到当前题意，不要反过来用模板猜题。
//
// 6. 核心算法
