...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：这里的压栈顺序与 LC-144 正好相反：为了生成根-右-左，要先 push(left) 再 push(right)。
//
// 4. 从具体问题抽象规律
//   这是典型的“目标顺序难直接维护 -> 构造一个容易的逆序 -> 最后统一变换”。
//
// 5. 专项算法｜后序遍历：先构造根-右-左，再整体反转成左-右-根
//   到这里才需要给这套规律命名：本题对应“后序遍历：先构造根-右-左，再整体反转成左-右-根”。通用机制见 docs/algorithms/tree-traversal.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
