...
//
// 1. 题目直觉 / 图形模型
//   把有环链表想成“直道 + 环形跑道”：╲n╲nhead -> a -> b -> c -> d╲n                 ^         ╲╲╲n                 |          e╲n                 +---- g <- f╲n╲nslow 每轮走 1 步，fast 每轮走 2 步。╲n在直道上，两者只是前后移动；一旦 slow 也进入环，就等价于两个人在圆形跑道上追逐。╲n╲n此时关键不再是“fast 的绝对位置”，而是 fast 相对 slow 每轮多走 1 格：╲n╲n相对距离：0, 1, 2, 3, ...  (mod 环长)╲n╲n所以只要存在环，相对距离迟早会再次变成 0，也就是两指针相遇。╲n无环则完全不同：fast 只会一路冲到 nullptr。
//
// 2. 最自然的一般性解法
//   最自然的判环方法，是边走边把访问过的节点地址放进 unordered_set；某个节点第二次出现就说明有环。这个方法直观且 O(n)，但需要 O(n) 额外空间。
//
// 3. 一般解法的问题与限制
...
//
// 5. 专项算法｜Floyd 快慢指针：用速度差发现周期
//   到这里才需要给这套规律命名：本题对应“Floyd 快慢指针：用速度差发现周期”。通用机制见 docs/algorithms/linked-list-pointer.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
