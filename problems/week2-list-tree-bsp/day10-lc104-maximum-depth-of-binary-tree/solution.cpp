...
//
// 2. 最自然的一般性解法
//   树形递归的第一原则：先写清“这个函数给父节点返回什么”，再写如何由孩子返回值合成自己。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：本题深度按“节点数”计，所以空树 0、叶子 1。
//
// 4. 从具体问题抽象规律
...
//   到这里才需要给这套规律命名：本题对应“树形递归：先定义子树返回值，再由左右答案合成当前节点”。通用机制见 docs/algorithms/tree-traversal.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 7. 核心算法
