...
//
// 4. 从具体问题抽象规律
//   这条推导比直接背双指针更重要：先把 O(n^2) 重复边界查询变成前后缀缓存，再问缓存中哪些信息可以在线延续、何时一侧答案已被较小边界确定。
//
// 5. 专项算法｜双侧边界压缩：从每格左右扫描到 O(1) 双指针
//   到这里才需要给这套规律命名：本题对应“双侧边界压缩：从每格左右扫描到 O(1) 双指针”。通用机制见 docs/algorithms/two-pointers.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
