...
//
// 5. 专项算法｜二维前缀和 + 容斥
//   到这里才需要给这套规律命名：本题对应“二维前缀和 + 容斥”。通用机制见 docs/algorithms/prefix-sum-difference.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
...
//
// 9. 为什么不会漏 / 不会重
//   大左上矩形覆盖了目标区以及上方、左侧多余区域；减去上方和左侧后，二者交集被减了两次，所以加回一次。每个目标单元最终保留一次，其他单元最终系数为 0。
//
// 10. 边界与易错点
