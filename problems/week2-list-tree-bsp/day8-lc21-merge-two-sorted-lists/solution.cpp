...
//
// 2. 最自然的一般性解法
//   维护结果链表尾部。只要两条链都非空，就比较当前头节点，把较小者接到结果末尾并推进它所属链表；一条耗尽后，另一条剩余后缀已经有序，可以整体接上。
//
// 3. 一般解法的问题与限制
//   这个逻辑本身已经是线性最优的；主要工程问题是第一次接节点时结果头尚不存在，以及相等值如何处理。若为头节点单独写分支，会让同一连接逻辑出现两套。
//
// 4. 从具体问题抽象规律
//   两个输入各自有序意味着：每一步全局最小候选只可能出现在各自当前头部。虚拟头节点可以把‘第一次连接’和后续连接统一成同一种 tail->next 操作。
//
// 5. 专项算法｜哑节点 + 双路归并
//   前面的推理已经得到可复用机制；现在才给它一个检索名称：有序链表归并 + dummy。通用机制见 docs/algorithms/linked-list-pointer.md。回到本题时，只需要把其中的状态、边界和更新事件映射到当前题意，不要反过来用模板猜题。
//
// 6. 核心算法
