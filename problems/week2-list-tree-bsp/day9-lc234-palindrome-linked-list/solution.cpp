...
//
// 1. 题目直觉 / 图形模型
//   数组判断回文可以左右夹逼，但单链表不能从尾部向前走：╲n╲n1 -> 2 -> 3 -> 2 -> 1╲n^                   ^╲nleft             想要 right，但拿不到前驱╲n╲n所以把“从尾往前”改造成“把后半段翻过来后从前往后”：╲n╲n原链：   1 -> 2 -> 3 -> 2 -> 1╲n                    slow╲n╲n反转 slow 开始的后半段后：╲nleft:    1 -> 2 -> 3 ...╲nright:   1 -> 2 -> 3╲n╲n现在两个指针都只需沿 next 向右，就在比较原链的镜像位置。╲n这里其实没有新指针技巧：找中点来自 fast/slow，反转完全复用 Day 8 的 LC-206。
//
// 2. 最自然的一般性解法
...
//   比较完成后再反转一次恢复结构。
//
// 5. 专项算法｜快慢指针找中点 + 复用 LC-206 反转 + 同向比较
//   到这里才需要给这套规律命名：本题对应“快慢指针找中点 + 复用 LC-206 反转 + 同向比较”。通用机制见 docs/algorithms/linked-list-pointer.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
