...
//
// 2. 最自然的一般性解法
//   最容易理解的做法，是先定位 [left,right]，把这段节点值或节点指针暂存到数组/栈里，再按逆序写回或重新连接；时间 O(n)，但需要 O(right-left) 额外空间。
//
// 3. 一般解法的问题与限制
...
//
// 5. 专项算法｜哑节点 + 头插法反转局部区间
//   到这里才需要给这套规律命名：本题对应“哑节点 + 头插法反转局部区间”。通用机制见 docs/algorithms/linked-list-pointer.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
...
//
// 10. 边界与易错点
//   这里最容易错的是连接顺序：必须先让 first->next 跳过 moved，否则 moved 被插到前面后会形成错误链；循环次数是 right-left，不是区间长度。dummy 的价值是把“反转从头开始”变成普通情况。
//
// 11. 举一反三
//   直接建立在 LC-206 的“改 next 前先保住后路”之上，只是 LC-206 让 previous 向前生长，LC-92 则固定 before，用连续头插完成局部重排。K 组翻转和链表局部旋转都会复用这种边界节点思想。
// ----------------------------------------------------------------------------
//
// 本地输入输出格式（用于 test.in）：
//   第 1 行：n left right。
