...
//
// 6. 专项算法｜前缀可达 DP：把指数分割搜索压缩成字符串位置上的 DAG 可达性
//   到这里才需要给这套规律命名：本题对应“前缀可达 DP：把指数分割搜索压缩成字符串位置上的 DAG 可达性”。通用机制见 docs/algorithms/dynamic-programming.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 7. 核心算法
