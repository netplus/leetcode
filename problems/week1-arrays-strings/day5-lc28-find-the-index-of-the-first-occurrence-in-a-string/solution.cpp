...
//
// 5. 专项算法｜KMP：失配时复用已匹配前缀
//   到这里才需要给这套规律命名：本题对应“KMP：失配时复用已匹配前缀”。通用机制见 docs/algorithms/kmp.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
...
//
// 9. 为什么不会漏 / 不会重
//   当 j 个字符已经匹配后发生失配，lps[j-1] 对应的前缀与这段已匹配文本的后缀完全相同，因此这些字符无需再次与文本比较；所有可能更长的边界都已被排除。文本指针从不回退，每次模式回退又严格缩短 j，所以总复杂度是 O(n+m)。
//
// 10. 边界与易错点
//   lps 表示最长真前后缀，不包含整个字符串自身；构造 lps 时失配也可能连续回退多次，不能直接清零；文本失配且 j>0 时不能推进 i，否则会跳过潜在匹配。
//
// 11. 举一反三
//   KMP 的核心不是背代码，而是“状态失败后退到仍然成立的最长边界”。字符串周期、重复子串、前后缀边界问题都在复用同一个 lps 状态。
// ----------------------------------------------------------------------------
//
// 本地输入输出格式（用于 test.in）：
//   第 1 行：haystack。
