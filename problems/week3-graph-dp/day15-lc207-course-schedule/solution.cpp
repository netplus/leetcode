...
//
// 2. 最自然的一般性解法
//   课程表首先是一个有向图建模题：prerequisites=[a,b] 表示 b 必须先于 a，因此边必须是 b->a。能否完成全部课程，等价于依赖图是否无环。Kahn 算法不直接寻找环，而是不断拿走当前没有剩余前置依赖的节点；若最后仍有节点拿不走，剩余部分就被环互相卡住。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：最容易错的是边方向：题目给 [a,b]，必须建立 b->a。
//
// 4. 从具体问题抽象规律
//   从 LC-200/133 的“遍历可到达节点”升级到“遍历必须遵守依赖顺序”。
//
// 5. 必要背景知识
//   对有向图中的边 u -> v：
...
//   到这里才需要给这套规律命名：本题对应“拓扑排序：把“先修依赖”变成有向边，反复删除当前入度为 0 的课程”。通用机制见 docs/algorithms/graph-search-toposort.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 7. 核心算法
