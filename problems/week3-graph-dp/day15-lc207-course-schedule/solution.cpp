...
//
// 2. 最自然的一般性解法
//   一个直观做法是反复扫描所有尚未完成课程，寻找‘所有前置课都已完成’的课程并把它们完成；若某一轮一个都找不到但仍有课程剩余，就说明依赖互相卡住。
//
// 3. 一般解法的问题与限制
//   反复全表扫描会一遍遍检查同一批尚未满足的前置条件，最坏产生大量重复工作。其实每完成一门课，只会影响它直接指向的后续课程；无需重新检查无关节点。
//
// 4. 从具体问题抽象规律
//   把每门课尚未满足的前置数压成一个计数。计数降到 0 的瞬间，课程就从‘不可处理’变成‘可处理’，再只通知它的直接后继。若最终仍有节点从未变成 0，则存在循环依赖。
//
// 5. 必要背景知识
//   对有向图中的边 u -> v：
...
//
// 6. 专项算法｜拓扑排序：把“先修依赖”变成有向边，反复删除当前入度为 0 的课程
//   前面的推理已经得到可复用机制；现在才给它一个检索名称：入度驱动的拓扑排序。通用机制见 docs/algorithms/graph-search-toposort.md。回到本题时，只需要把其中的状态、边界和更新事件映射到当前题意，不要反过来用模板猜题。
//
// 7. 核心算法
