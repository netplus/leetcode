...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：最危险的是先写 current->next=previous 再取原 next，这会直接丢失剩余链表；返回值应是 previous，而不是已经变成尾节点的原 head。
//
// 4. 从具体问题抽象规律
//   这是本周最基础的链表指针模型。
//
// 5. 专项算法｜链表原地反转：三指针
//   到这里才需要给这套规律命名：本题对应“链表原地反转：三指针”。通用机制见 docs/algorithms/linked-list-pointer.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
