...
- Each header presents the title, statement, constraints, complexity goal, local I/O contract, and expected output in Chinese. Problems that have received an individual learning rewrite follow `AGENTS.md`: **图像直觉 → 一句话核心 → 公式/不变量 → 执行步骤 → 正确性直觉 → 易错点 → 迁移**. Untouched problems retain the existing reviewed explanation until they are individually optimized.
- `tools/refined_week1.py` through `tools/refined_week4.py` are the baseline reviewed explanation/implementation sources. `tools/pedagogy_overrides.py` stores only individually optimized learning rewrites (and implementation overrides when the clearer primary solution changes). `tools/refined_data.py` merges both layers. `python3 tools/gen_all.py` regenerates reviewed solutions without erasing those rewrites.
