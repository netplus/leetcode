...
//
// 4. 从具体问题抽象规律
//   与 Day 25 的 Top-K/边界维护不同，这里进一步问‘能否不模拟每一步，只保留决定最优值的紧约束’。
//
// 5. 必要背景知识
//   任务调度器里不能只记“一个最高频任务”的频次，还要记有多少种任务并列最高频。
//
//   反例：tasks=AAABBB, n=2。
...
//   `(f-1)*(n+1)+maxCount`。
//
// 6. 专项算法｜频率骨架：由最拥挤任务给出调度长度下界
//   到这里才需要给这套规律命名：本题对应“频率骨架：由最拥挤任务给出调度长度下界”。通用机制见 docs/algorithms/greedy.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 7. 核心算法
