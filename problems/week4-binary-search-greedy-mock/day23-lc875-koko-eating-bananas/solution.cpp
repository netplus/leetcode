...
//
// 5. 必要背景知识
//   不能把最低速度简化成 `ceil(sum(piles)/h)` 这样的平均吞吐，因为 Koko 每小时只能选择一堆；一堆提前吃完后，本小时剩余能力不能转移到另一堆。
//
...
//
// 6. 专项算法｜答案二分：把最小吃速转成速度轴上的第一个可行点
//   到这里才需要给这套规律命名：本题对应“答案二分：把最小吃速转成速度轴上的第一个可行点”。通用机制见 docs/algorithms/binary-search.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 7. 核心算法
