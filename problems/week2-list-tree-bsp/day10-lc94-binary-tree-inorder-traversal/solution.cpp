...
//
// 2. 最自然的一般性解法
//   把显式栈理解为递归调用栈：沿左链压栈就是不断调用 left；弹栈就是一次 left 调用返回。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：外层条件必须是 current || !stack.empty()；否则 current 变成 null 时会提前结束，来不及恢复祖先。
//
// 4. 从具体问题抽象规律
//   LC-144 的栈保存“未来子树”，LC-94 的栈保存“暂停的祖先现场”。
//
// 5. 专项算法｜中序遍历：栈保存“左子树回来后还要处理的祖先”
//   到这里才需要给这套规律命名：本题对应“中序遍历：栈保存“左子树回来后还要处理的祖先””。通用机制见 docs/algorithms/tree-traversal.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
