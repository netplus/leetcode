...
//
// 2. 最自然的一般性解法
//   图深拷贝的难点不是 BFS，而是环和共享邻居：同一个原节点可能从多条边被再次遇到，但克隆图里必须始终对应同一个新对象。因此需要一张 original Node* -> clone Node* 的映射。它同时承担 visited 和对象身份索引两个角色。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：关键是先登记映射再入队，否则环可能导致重复创建。
//
// 4. 从具体问题抽象规律
//   它和带 random 指针链表、对象依赖图、AST/IR 克隆本质相同：先建立 old->new 身份映射，再按原引用关系重建新引用。
//
// 5. 必要背景知识
...
//
// 6. 专项算法｜图的深拷贝：遍历负责发现结构，old->new 映射负责保持对象身份
//   到这里才需要给这套规律命名：本题对应“图的深拷贝：遍历负责发现结构，old->new 映射负责保持对象身份”。通用机制见 docs/algorithms/graph-search-toposort.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 7. 核心算法
