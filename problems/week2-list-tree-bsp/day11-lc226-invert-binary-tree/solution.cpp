...
//
// 2. 最自然的一般性解法
//   复用 Day 10 的递归职责模型：子树先返回一个完整结果，父节点只负责组合，不跨层操心子树内部。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：如果直接写 root->left = invertTree(root->right) 后又基于已经改过的 root->left 去处理另一边，可能覆盖原指针；本实现先把两个递归结果保存到局部变量最清晰。
//
// 4. 从具体问题抽象规律
//   这与 Day 10 LC-105“左右子树先递归构造，再挂回根”是同一种组合式递归。
//
// 5. 专项算法｜树结构变换：先让左右子树完成职责，再交换当前节点的两个结果
//   到这里才需要给这套规律命名：本题对应“树结构变换：先让左右子树完成职责，再交换当前节点的两个结果”。通用机制见 docs/algorithms/tree-traversal.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
