...
//
// 2. 最自然的一般性解法
//   把 queue.size() 当成当前 BFS frontier 的边界快照；新产生的 frontier 留到下一轮。
//
//   按实现顺序可以直接写成：
...
//
// 3. 一般解法的问题与限制
//   这道题的一般性逻辑本身已经接近目标复杂度，没有必要为了形式统一而人为制造更慢的“暴力解”。这里真正需要约束的是状态定义、边界和更新顺序：不能写成 for (i=0; i<pending.size(); ++i)，因为 size 在循环中会随着孩子入队变化，层边界会被破坏。
//
// 4. 从具体问题抽象规律
//   这一题建立的 frontier 模型会直接迁移到 Week 3 图 BFS：腐烂橘子、01 矩阵、多源 BFS 都是在一层层推进“距离”。
//
// 5. 专项算法｜层序遍历：BFS 队列 + 冻结当前层宽度
//   到这里才需要给这套规律命名：本题对应“层序遍历：BFS 队列 + 冻结当前层宽度”。通用机制见 docs/algorithms/tree-traversal.md；回到本题时只保留与当前题意直接相关的状态、边界和更新顺序。
//
// 6. 核心算法
